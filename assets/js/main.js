...
let carousel__actions = document.querySelectorAll('.carousel__actions');
...
const repeat = function(activeClass){
    ...
    let repeater = () => {
      setTimeout(function(){
        [...carousel__item].forEach((activeSlide) => {
          activeSlide.classList.remove('carousel__item--visible')
        });
...
const calculateBmi = (e) => {
    ...
    if (calculateCm.value === '' || calculateKg === '') {
        ...
        if (bmi < 18.5) {
            ...
            calculateMessage.textContent = `Seus IMC é de ${bmi} e você está magro`
        } else if (bmi < 25) {
            calculateMessage.classList.add('color-green')
            calculateMessage.textContent = `Seu IMC é de ${bmi} você está saudável`
        } else {
            calculateMessage.classList.add('color-green')
            calculateMessage.textContent = `Seu IMC é de ${bmi} você está acima do peso`
        }
...
const sendEmail = (e) => {
    ...
    if (contactUser.value === '') {
        ...
        // Show message
        contactMessage.textContent = 'Por favor coloque seu e-mail!'
